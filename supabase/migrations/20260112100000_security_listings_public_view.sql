...
-- ============================================================================
-- This migration:
-- 1. Creates a public view (listings_public) that excludes sensitive fields
-- 2. REMOVES all public SELECT from base listings table (admin only)
-- 3. Creates SECURITY DEFINER function for single-listing access (detail pages)
-- 4. Updates RPC functions to exclude sensitive fields
-- 
-- After this migration:
-- - Bulk queries: Must use listings_public view (no contact data)
-- - Single listing detail: Must use get_listing_by_slug() RPC (includes contact data)
...
-- ============================================================================
-- This view is what all list/search queries should use
-- It includes everything needed for category pages, search, venues list
-- but EXCLUDES: email, phone, contact_name, contact_email, contact_phone, box_office_phone
...
-- ============================================================================
-- Remove ALL public read access from the base table
-- This is the core security change - no more bulk extraction possible

DROP POLICY IF EXISTS "Active listings are publicly readable" ON public.listings;

-- The existing "Admins can view all listings" policy remains, so admins still have full access
-- No new public policy is created - anonymous users cannot query the base table at all

-- ============================================================================
-- STEP 3: Create SECURITY DEFINER function for single-listing detail pages
-- ============================================================================
-- This function bypasses RLS to return full listing data including contact fields
-- It's safe because:
-- - Only returns ONE record (LIMIT 1)
-- - Requires exact slug match
...
-- Grant execute to all users (the function itself controls what data is returned)
GRANT EXECUTE ON FUNCTION public.get_listing_by_slug(text) TO anon, authenticated;

-- ============================================================================
-- STEP 4: Update get_listings_by_category RPC to exclude sensitive fields
-- ============================================================================
-- This function is used by category pages - no contact data needed
...
RETURNS TABLE(
  ...
  youtube_url text,
  tiktok_url text
  -- REMOVED: email, phone (sensitive data)
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
-- ============================================================================
-- STEP 5: Update get_all_listings_with_categories to exclude sensitive fields
-- ============================================================================
-- Used for admin export/data operations - now excludes contact data for safety
...
RETURNS TABLE (
  ...
  keywords text,
  website text,
  -- REMOVED: email, phone, contact_name, contact_email, contact_phone, box_office_phone
  address text,
  town_city text,
  county text,
  postcode text,
